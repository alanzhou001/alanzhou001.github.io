...
$$
where $f(x^*)$ is the optimal value of ($\ref{opt1}$).

Some special cases:

- *infeasible(不可行)：* $X = \empty$;
- *unbounded(无界)：*$(x^n)_{n=1}^{\infin}\in X$, $f(x^n)\to \infin$;
...
1. The space $\mathbb{R}^n$
   ...
   - **scalar multiplication(数乘)** 
     ...
     $a\in \mathbb{R}, x \in \mathbb{R}^n, ax=(ax_1,ax_2,\cdots,ax_n)$
   ...
   **Proof**. **(重复加入/减去线性无关向量直到构成基)**

   **Proposition:** Let $\mathcal{W}$ be a vector space spanned by a finite set $\mathcal{V}$, i.e., $\mathcal{W}=span(\mathcal{V})$. Any linearly independent subset of $\mathcal{W}$ can be extended to form a basis for $\mathcal{W}$ .
   ...
   **Proposition:** Any finite set $\mathcal{V}$ that spans a vector space W can be reduced to a basis for $\mathcal{W}$ by removing some of its vectors.
